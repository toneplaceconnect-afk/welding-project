...
Полное CMS-покрытие текстов: 2026-09-16 (v4)
...
## CMS v4: полное редактирование текстов (2026-09-16)

Все видимые тексты сайта редактируются из админки (`admin.html` → `content.json` → автодеплой Vercel):

- **Глобальное**: бренд, телефоны, email, Telegram, футер, меню шапки (`nav`), ссылки футера, плашка «ЗВОНИТЕ СЕЙЧАС» — `site-shell.js` строит шапку/футер из CMS с fallback.
- **Главная**: hero, вступление, карточки, направления (тексты+фото), калькулятор-блок, «создай своё», лофт-блок, заявка (включая опции списка и подсказки полей) — `applyHome` + `form_fields`.
- **Каталог**: hero, «почему», изделия (тексты+фото), процесс — `applyLoft` обновляет карточки по месту.
- **Цены**: hero, заголовки и строки таблицы (9+2), CTA — `applyPrices` по индексу строк.
- **Документация**: hero, баннер, группы норм — через `renderVals` + `{{ }}` + `setState` (шаблон уже на `sc-for`).
- **Калькулятор**: hero, подписи интерфейса, нормы, названия категорий/полей/вариантов — мерж CMS в `CATS` + `setState`. Значения по умолчанию, типы полей и формулы расчёта — код (не редактируются).
- **Создай своё**: все тексты — `applyCreate`.

Найденные и исправленные ошибки:
- `applyPage` вызывалась, но не была определена → `ReferenceError` глушил все текстовые применения и `applyGlobal`. Восстановлена с картой `{home, loft, prices, calculator, create}`.
- Селекторы `section[style*="min-height"] h1/p` не попадали в hero каталога и прайса → заменены на ID (`loft-hero-title/text`, `prices-hero-title/text`).
- `pages.loft.hero_images` / `pages.home.hero_images` писались админкой, но никем не читались → `applyLoft`/`applyHome` читают массив (первое фото = главное).
- Загрузка фото в карточку дописывала файл в `images` вместо замены `images[0]` → ложные галереи. Исправлено в `admin.html`.
- Галерея использовала `scrollIntoView` и тащила страницу → скролл только ленты (`track.scrollTo`).
- `gallery.js` писал галерею в light DOM внутри Shadow DOM `image-slot` (невидимо) → слот заменяется на `div.gallery`.
- Порядок `applyAll`: `applyGalleryImages` до `applyPage`, чтобы данные раздела побеждали legacy-карту.

Границы CMS (остаются кодом): формулы и значения калькулятора, `hint-placeholder` стриминга, техошибки API.
