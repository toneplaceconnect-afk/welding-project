...
function dataUrlToBlob(dataUrl, index) {
  ...
  if (!m) throw Error('╨¥╨╡╨║╨╛╤Ç╤Ç╨╡╨║╤é╨╜╤ï╨╣ ╤ä╨╛╤Ç╨╝╨░╤é ╨╕╨╖╨╛╨▒╤Ç╨░╨╢╨╡╨╜╨╕╤Å Γäû' + (index + 1));
...
async function analyzeBrief(clientBrief, accountId, token) {
  ...
  if (!r.ok) {
    ...
    throw Error('╨í╨╡╤Ç╨▓╨╕╤ü ╨░╨╜╨░╨╗╨╕╨╖╨░ ╨▓╤Ç╨╡╨╝╨╡╨╜╨╜╨╛ ╨╜╨╡╨┤╨╛╤ü╤é╤â╨┐╨╡╨╜ (HTTP ' + r.status + '). ' + errBody.slice(0, 200));
  }
...
async function generateImage(prompt, refs, accountId, token) {
  ...
  if (!r.ok) {
    ...
    if (r.status === 403 || r.status === 429) {
      throw Error('╨í╨╡╤Ç╨▓╨╕╤ü ╨│╨╡╨╜╨╡╤Ç╨░╤å╨╕╨╕ ╨▓╤Ç╨╡╨╝╨╡╨╜╨╜╨╛ ╨╜╨╡╨┤╨╛╤ü╤é╤â╨┐╨╡╨╜ (HTTP ' + r.status + '). ╨ƒ╤Ç╨╛╨▓╨╡╤Ç╤î╤é╨╡ ╨┤╨╜╨╡╨▓╨╜╨╛╨╣ ╨╗╨╕╨╝╨╕╤é ╨╕ ╨┐╨╛╨┐╤Ç╨╛╨▒╤â╨╣╤é╨╡ ╨┐╨╛╨╖╨╢╨╡.');
    }
    throw Error('╨í╨╡╤Ç╨▓╨╕╤ü ╨│╨╡╨╜╨╡╤Ç╨░╤å╨╕╨╕ ╨▓╤Ç╨╡╨╝╨╡╨╜╨╜╨╛ ╨╜╨╡╨┤╨╛╤ü╤é╤â╨┐╨╡╨╜ (HTTP ' + r.status + '). ' + errBody.slice(0, 200));
  }

  const d = await r.json();
  const image = d?.result?.image;
  if (!image) throw Error('Cloudflare ╨╜╨╡ ╨▓╨╡╤Ç╨╜╤â╨╗ ╨╕╨╖╨╛╨▒╤Ç╨░╨╢╨╡╨╜╨╕╨╡.');
...
export default async function handler(request, response) {
  ...
  if (!accountId || !token) {
    return response.status(500).json({
      error: '╨¥╨╡ ╨╜╨░╤ü╤é╤Ç╨╛╨╡╨╜ Cloudflare Workers AI. ╨¥╤â╨╢╨╜╤ï ╨┐╨╡╤Ç╨╡╨╝╨╡╨╜╨╜╤ï╨╡ CLOUDFLARE_ACCOUNT_ID ╨╕ CLOUDFLARE_API_TOKEN ╨▓ Vercel.'
    });
  }
  ...
  if (!clientPrompt) return response.status(400).json({ error: '╨ƒ╤â╤ü╤é╨╛╨╣ ╨╖╨░╨┐╤Ç╨╛╤ü (prompt).' });
  if (clientPrompt.length > 3000) return response.status(400).json({ error: '╨₧╨┐╨╕╤ü╨░╨╜╨╕╨╡ ╤ü╨╗╨╕╤ê╨║╨╛╨╝ ╨┤╨╗╨╕╨╜╨╜╨╛╨╡. ╨£╨░╨║╤ü╨╕╨╝╤â╨╝ 3000 ╤ü╨╕╨╝╨▓╨╛╨╗╨╛╨▓.' });
  if (refs.length > 2) return response.status(400).json({ error: '╨£╨░╨║╤ü╨╕╨╝╤â╨╝ 2 ╨╕╨╖╨╛╨▒╤Ç╨░╨╢╨╡╨╜╨╕╤Å.' });
  ...
  try {
    ...
    const prompt = [
      ...
      visualSpec,
      'ORIGINAL CLIENT BRIEF ΓÇö FINAL AUTHORITY:',
      clientPrompt
    ].join('\n\n');
    ...
    return response.status(500).json({ error: '╨₧╤ê╨╕╨▒╨║╨░ ╨│╨╡╨╜╨╡╤Ç╨░╤å╨╕╨╕: ' + (err?.message || String(err)) });
  }
}
