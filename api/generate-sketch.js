...
COMPONENT-BASED CONSTRUCTION: Treat the product as an assembly of separate physical components. Distinguish overall product dimensions from component dimensions and cross-sections. A component dimension describes that component only.

CRITICAL UNIT RULE: A dimension such as 100x150 mm attached to a timber beam means beam cross-section 100x150 mm, NOT that the tabletop or whole product is 100x150 mm. A dimension such as 150x150 mm attached to a beam means beam cross-section 150x150 mm. Never confuse overall product dimensions with component cross-sections.
...
`VIEW B — SAME EXACT PHYSICAL PRODUCT, DIFFERENT CAMERA. Use the canonical product image as the strongest identity reference. Reproduce the exact same physical object: identical tabletop/body shape, dimensions, thickness, number and shape of legs/supports, joints, materials, colors, finish and proportions. Change ONLY camera position, viewing angle, crop and visible sides. Do not redesign, simplify, add or remove any physical component.`,
...
export default async function handler(request,response){
 ...
 try{
  ...
  // Upload the canonical image and use it as the dominant reference for EVERY subsequent view.
  // This is deliberately stronger than sending the original client references again, because
  // the first generated image is now the single source of truth for multi-view consistency.
  let canonicalUrl=null;
  try{canonicalUrl=await uploadImage(first,0,apiKey)}catch(e){console.warn('Could not upload canonical image:',e)}
  const identityRefs=canonicalUrl?[canonicalUrl]:refs;

  for(let i=1;i<count;i++){
   const viewPrompt=makePrompt(i);
   let image;
   try{
    image=await generate(viewPrompt,identityRefs,apiKey);
   }catch(e){
    image=await fallback(viewPrompt+'\nThe canonical first image is the authoritative design reference. Reproduce that exact object; only change the camera.',apiKey);
   }
   images.push(image);
  }
